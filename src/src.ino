      ...
      if (!animation->drawConfirmAnimationFrame()) {
        ...
        case UiState::BLUETOOTH:
          animation->drawBluetoothScreen(
              BleManager::getDeviceName(), BleManager::isAdvertising(),
              BleManager::isConnected(), BleManager::getConnectedClients(),
...
void loop() {
  ...
  OtaManager::update();
  BleManager::update();
  // Obsluga zapytan do serwera WWW
  AkwariumWifi::getServer().handleClient();
