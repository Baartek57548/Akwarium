...
static void syncTestOverrideWithUiState() {
  if (!animation) {
    return;
  }

  if (uiState == UiState::TESTS) {
    SystemController::setTestOverride(animation->getTestLight(),
                                      animation->getTestHeater(),
                                      animation->getTestFilter(),
                                      animation->getTestAeration());
  } else {
    SystemController::clearTestOverride();
  }
}
...
static void applyPendingUiChanges() {
  ...
  if (applyTime) {
    ...
    uint32_t epochUtc = TimeUtils::localDateTimeToUtcEpoch(localDateTime);
    syncSystemTime(epochUtc);
  }
}
...
void updateUiState() {
  ...
  bool anyButtonPressed = isUpPressed || isSelectPressed || isDownPressed;
  ...
  if (anyButtonPressed) {
    // Po wygaszeniu OLED pierwszy przycisk ma tylko obudzic ekran.
  ...
  case UiState::MENU:
    ...
    if (selectJustPressed) {
      ...
      if (sel == 0) {
        ...
        SystemController::runFeederCalibrationFromMenu(&display);
        uiState = UiState::MENU;
      } else if (sel == 5) {
