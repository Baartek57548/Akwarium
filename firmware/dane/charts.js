...
(() => {
    ...
    window.ChartsApp = {
        updateData: function(temperatureData) {
            allData = [];
            const history = temperatureData.history || [];
            ...
            if (activeRange === '1h' && allData.length < 6) activeRange = '3h';
            
            // Re-render if section is active (could optimize further based on display: none)
    ...
    function changeRange(range) {
        ...
        dom.chartSubtitle.textContent = rangeLabels[range] || '';
        hoverIndex = -1;
